...
namespace OpenDK
{
	...
	std::size_t ColumnFile::getSize() const
	{
		return size;
	}
	...
	bool ColumnFile::cubeIsSolid(int columnIndex, int height) const
	{
		...
		std::size_t offset = (columnIndex * CHUNK_SIZE) + 3;
	...
	{
		std::size_t offset = (columnIndex * CHUNK_SIZE) + 5;
	...
	{
		std::size_t offset = (columnIndex * CHUNK_SIZE) + 0;
	...
	{
		std::size_t offset = (columnIndex * CHUNK_SIZE) + 2;
