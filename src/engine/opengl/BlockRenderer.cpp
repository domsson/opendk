...
namespace OpenDK
{
	...
	void BlockRenderer::moveLight(float offsetX, float offsetY, float offsetZ)
	{
	...
	void BlockRenderer::moveLight(float offsetX, float offsetZ)
	{
		glm::vec3 pos = light.getPosition();
	...
	void BlockRenderer::moveCam(float offsetX, float offsetY, float offsetZ)
	{
	...
	void BlockRenderer::rotateCam(float rotX, float rotY, float rotZ)
	{
		glm::vec3 rot = camera.getRotation();
		camera.setRotation(glm::vec3(rot.x + rotX, rot.y + rotY, rot.z + rotZ));
	}

	void BlockRenderer::zoomCam(float zoomChange)
	{
		camera.setZoom(camera.getZoom() + zoomChange);
	}

	void BlockRenderer::debugCamCoords() const
	{
	...
	void BlockRenderer::debugLight() const
	{
...
*/
	...
	void BlockRenderer::debugDAT() const
	{
		for (int y = 0; y < 85; ++y)
		{
			for (int x = 0; x < 85; ++x)
	...
	void BlockRenderer::bakeLight(const Light& l)
	{
	...
	void BlockRenderer::updateLight()
	{
	...
	void BlockRenderer::renderBlock(const VertexArrayObject& vao, int slabX, int slabZ)
	{
		int x = slabX * 3;
		int z = slabZ * 3;
	...
	float BlockRenderer::getLightLevelAt(int x, int z, int side) const
	{
		int i;
	...
	bool BlockRenderer::bresenham(int x1, int y1, int const x2, int const y2, int lightHeight)
	{
		int delta_x(x2 - x1);
